...
int main(int argc, char *argv[]){
    ...
    core_init();
    main_menu_init(NULL);
    void *pmotorcicle_data = NULL;

    // extern int bmp_rain2(void); bmp_rain2();
    ...
    while (1) {        
        ...
        if(NULL == pmotorcicle_data)
            pmotorcicle_data = init_draw_rotate();
        ldraw_rotate(pmotorcicle_data);
        // demo defs       
        render_demo_mode();
        draw_alpha_line();
    ...
    main_menu_deinit();
    if(NULL != pmotorcicle_data)
        deinit_draw_rotate(pmotorcicle_data);
    core_deinit();
    return 0;
}
